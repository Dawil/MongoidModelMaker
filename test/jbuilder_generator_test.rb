...
module MongoidModelMaker
  class JBuilderTest < Rails::Generators::TestCase
    ...
    test "works on a model with no fields" do
      ...
      assert_file "app/views/people/_person.json.jbuilder", <<-RUBY.gsub(/^#{'  ' * 3}/,'')
      json.id person.id
    ...
    test "calls partial from index page" do
      @file_helper.create_file "tmp/app/views/people/index.json.jbuilder", <<-RUBY.gsub(/^#{'  ' * 3}/,'')
      ...
      RUBY
      run_generator %w(person first:string last:string)
      assert_file "app/views/people/index.json.jbuilder", <<-RUBY.gsub(/^#{'  ' * 3}/,'')
      json.array!(@people) do |person|
    ...
    test "calls partial from show page" do
      @file_helper.create_file "tmp/app/views/people/show.json.jbuilder", <<-RUBY.gsub(/^#{'  ' * 3}/,'')
      json.extract! @person, :created_at, :updated_at, :first, :last
      RUBY
      run_generator %w(person first:string last:string)
      assert_file "app/views/people/show.json.jbuilder", <<-RUBY.gsub(/^#{'  ' * 3}/,'')
      json.partial! "people/person.json.jbuilder", person: @person
      RUBY
    end

    test "adds a singular reference to partial in parent" do
      @file_helper.create_file "tmp/app/views/people/_person.json.jbuilder", <<-RUBY.gsub(/^#{'  ' * 3}/,'')
      json.id person.id
      json.extract! person, :created_at, :updated_at, :first, :last
      RUBY
